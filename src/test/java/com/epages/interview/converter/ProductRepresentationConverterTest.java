...
public class ProductRepresentationConverterTest {
    ...
    public void shouldConvertToProductRepresentationOnSale() {
        // Given
        final Product shirt = a_product(1L, "Panama Tricot", 12.00, ON_SALE, a_brand("ADIDAS"));

        // When
        final ProductRepresentation actual = converter.convert(shirt);

        // Then
        final ProductRepresentation expected = ProductRepresentation.builder().id(1L).name("Panama Tricot").price(12.00).event("ON SALE").build();
    ...
    public void shouldConvertToProductRepresentation() {
        // Given
        final Product shirt = a_product(1L, "Panama Tricot", 12.00, NO_SALE, a_brand("ADIDAS"));

        // When
        final ProductRepresentation actual = converter.convert(shirt);

        // Then
        final ProductRepresentation expected = ProductRepresentation.builder().id(1L).name("Panama Tricot").price(12.00).build();
        assertThat(actual).isEqualTo(expected);
    }
}